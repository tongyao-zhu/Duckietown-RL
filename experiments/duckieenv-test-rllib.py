...
from duckietown_utils.env import launch_and_wrap_env, get_wrappers, launch_and_wrap_duckieenv
...
parser.add_argument('--map-name', default="map1", help="Specify the map")
...
if not (
    ...
    for i in range(1):
        ...
        while not done:
            ...
            if not cv2.imwrite(os.path.join("observed_images", "{}.jpg".format(steps)),img):
                assert 1==2
            if done:
